...
#
# Tests a binary SVM classifier for Neutral vs others
#
...
for row, v in data.iterrows():
    ...
    if v.labels != 'discard' and (v.labels == 1 or v.labels == 2 or v.labels==5 or v.labels==3):
        ...
        if (v.labels == 2 or v.labels == 1 or v.labels == 5):
            d = v
            d.labels = 0
...
X_train, y_train = make_imbalance(X_train, y_train, sampling_strategy={0: 720, 3: 720},random_state=RANDOM_STATE)
X_test, y_test = make_imbalance(X_test, y_test, sampling_strategy={0: 320, 3: 320},random_state=RANDOM_STATE)
