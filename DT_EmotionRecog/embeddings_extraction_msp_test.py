...
#
# Extracts test DeepTalk Embeddings from MSP-Pod and stores them as a pickle in
...
msp_test = read_parquet('./working_data/msp_test.pq')
#msp_train = read_parquet('./working_data/msp_train.pq')

labels = {'H':1,'S':2,'N':3,'A':5}
msp_test.replace({'EmoClass':labels},inplace=True)
...
for i in range(len(msp_test)):
    data['subject'].append(msp_test.iloc[i,2])
    data['labels'].append(msp_test.iloc[i,1])
    ...
    try:
        print('opening', msp_prefix + msp_test.iloc[i, 0])
        embeddings = demo_functions.run_DeepTalk_demo(ref_audio_path=(msp_prefix +msp_test.iloc[i, 0]))
    except:
        print('bad embedding')
        embeddings = None
...
data_df.to_pickle('./working_data/msp_test_extracted_embeddings.pk')