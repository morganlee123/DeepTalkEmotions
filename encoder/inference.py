...
# from encoder.models.model_baseline import SpeakerEncoder
...
def load_model(weights_fpath: Path, module_name, device=None):
    ...
    if(torch.cuda.is_available()):
        ...
        checkpoint = torch.load(weights_fpath, map_location=lambda storage, loc: storage)
...
def embed_frames_batch(frames_batch,**kwargs):
    """
    ...
    embed = _model.forward(frames,**kwargs).detach().cpu().numpy()
    return embed
...
def compute_partial_slices(n_samples, partial_utterance_n_frames=partials_n_frames,
    ...
    steps = int(np.floor(steps/frame_step)*frame_step) # This ensures last slice does not go outside data range
...
def embed_utterance(wav, using_partials=True, return_partials=False, **kwargs):
    ...
    #print(n_frames, partials_n_frames)
    ...
    wave_slices, mel_slices = compute_partial_slices(len(wav), min_pad_coverage=0.75)
    ...
    if(mel_n_channels==40):
        ...
        frames = get_frame_from_file(wav, win=win, inc=inc, sr=sampling_rate, n_channels=1, duration = None)
        frames = np.transpose(frames)
        if(frames.shape[0]<mel_slices[-1].stop): # This ensures that the number of frames in 'frames' corresponds to the expected melshapes
